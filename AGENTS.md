...
- **State:** `cargo fmt --check`, `cargo clippy --workspace --all-targets -- -D warnings`, and `cargo test --workspace` are green. 206 tests pass across 28 suites. CI moved to the `docker-runner-amd64` container runner and the `rolling` release workflow landed (ADR-020, 2026-09-03) — until the runner is registered and `comicrust-ci:latest` is built on the runner host, pushed workflows sit queued on that label.
...
- CI: Gitea Actions. Both workflows run on the `docker-runner-amd64` runner inside the `comicrust-ci` container image (`.gitea/container/Dockerfile`; build it on the runner host). `ci.yaml` runs fmt, clippy, and tests on every push to main (`CR_FORMAT_TESTS=1` — the image ships 7z and djvulibre). `release.yaml` builds `cr-app` in release mode and republishes the single `rolling` prerelease: version `0.0.<total commits on main>`, assets `comicrust-<version>-linux-amd64.tar.gz` + `.sha256` (binary renamed `comicrust`, plus `assets/papers/`). See ADR-020.
