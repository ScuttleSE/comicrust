...
Companion to `feasibility.md` (evidence) and `decisions.md` (locked scope). Estimates assume one experienced developer working ~full-time; divide durations accordingly for teams.
...
Crate split mirrors the C# project boundaries so porting stays mechanically traceable file-by-file, and `cr-cli` enables headless differential testing against real ComicRack data **before any UI exists**.
...
Every phase ends shippable/testable. Phases 0-2 are fully headless — they de-risk data compatibility with zero UI exposure.
...
1. **Data compat first (0-2):** the database is the only unlosable artifact; proving byte-stable round-trip before UI means the riskiest compat work happens while the codebase is small.
2. **Reader before browser (3 before 4):** the reader is the emotional core and validates the GL/cairo rendering strategy; the browser widget is the single largest custom build and benefits from the reader's widget infrastructure.
3. **Scripting late but before polish (6):** plugin API shape stabilizes only after the app surface exists; but it precedes polish because ecosystem compat may force host-API changes.
4. **Platform integration last (7):** sync/remote are isolated modules; deferring them avoids coupling their APIs to an unstable engine.
