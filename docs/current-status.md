...
Run the Phase 13 user test (the steps are in the phase file). No code work
is in flight.

## Verification record

- Commit: `f451af7` (2026-09-11).
- `cargo fmt --all` — green.
- `cargo clippy --workspace --all-targets -- -D warnings` — green.
- `cargo test --workspace` — 522 pass.
- Probes: cache (A-G), workspace (ALL PASS), scanrefresh (A-K, release
...
1. **Double-click open crash fix** (commit `140ba4c`) — double-click a book
...
2. **Phase 13 config unification** — the full steps are in
   `docs/phases/phase-13.md`.
3. **Config seed + reference doc** (commit `d5c52b3`) — the first start
...
4. **Mid-scan background save** (commit `d9262a4`) — start a scan of a large
   folder. Within about 10 minutes `~/.local/share/comicrust/ComicDb/
   ComicDb.xml` appears on disk and holds the books found so far.
5. **Smart-list rule delete and clipboard operations** — open a smart list
   ...
   stalls those reads.
6. **"No metadata" tag** — books whose scan found no metadata carry a small
...
7. **Scan and open metadata import** — rescan a folder that holds magazines
...
8. **Detail view round** (commits `c21086a`, `f20a690`) — switch the browser
...
9. **Group, lamp, and thumbnail batch** — Group by Series in Thumbnail,
...
10. **Scan control** — Tasks ▸ Abort Scanning on a real scan. Then a
...
11. **Export freeze fix** — re-run a CBR to CBZ export. The window stays
    responsive and the progress ticks.
12. **Write-back fix** — edit a property of a CBR or CB7 book, then run
    Update Book File(s). The UI stays responsive, the write lands, and the
    Files-to-update list clears.
13. **Phase 10 install and duplicate steps** — the steps at the tail of
    `docs/archive/phases/phase-10.md`.
14. **Phase 11 install steps** — the steps at the tail of
