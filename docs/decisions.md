...
Append-only. Add new ADRs at the end; never rewrite existing entries. Status: `accepted` / `superseded by ADR-nnn`.
...
- **Decision:** Read **and write** the existing XML format with exact element/attribute-name fidelity via serde + quick-xml. Verified by golden-file round-trip tests from Phase 0. No SQLite or format change without a new ADR.
- **Consequences:** Every model change must be validated against golden files; C# `XmlSerializer` quirks (ordering, attribute/element mix, empty-element forms) must be reproduced.
...
- **Decision:** Embed CPython 3 via PyO3, expose a `ComicRack`/`ComicBook` shim mirroring `IPluginEnvironment` (~40 methods). Keep `#@Directive`, manifest, and `.crplugin` formats unchanged. Ship a 2to3 migration guide; top-5 community plugins are acceptance tests.
- **Consequences:** Python 2 plugins need migration; scripts that build WinForms UI (`ComicInfoUI`/`QuickOpenUI` returning controls) are unportable — replaced by an HTML/WebKitGTK or GTK panel API (breaking change, documented).
...
- **Consequences:** Existing Android app will not work against comicrust. The wireless-sync raw TCP protocol (ports 7614/7615/7620+) is *separately* portable if device sync needs it.
...
- **Decision:** RAR/RAR5 via 7z or libarchive **subprocess** (as the C# app itself shells to 7z.exe) or pure-Rust readers if available; never statically link unrar.
- **Consequences:** Slight subprocess startup cost per RAR open; mitigated by persistent extraction process if needed.
...
- **Decision:** No LICENSE file yet. This must be resolved before any binary distribution; a future ADR will pick the license after reviewing upstream obligations.
- **Consequences:** Repo is private-planning until licensing lands; keep third-party dependency license inventory as we add crates.
...
- **Consequences:** Instant 19-language support; widget names in cr-ui should mirror the C# control names where translations must hit.
