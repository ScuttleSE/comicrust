...
- **Consequences:** A dispatch with a missing or malformed tag fails early, in the checkout or Version step. The release build checks out the tag, so the publish step derives the commit with `git rev-parse HEAD`. `github.sha` names the dispatching branch head, not the tag. `publish_release.sh` was checked offline with a stub `curl` only. The first real proof is a dispatch on the Gitea server. The image recipe now uses `--no-install-recommends`. The first image build failed on the invalid `--no-recommends` spelling.
